...
class Dataset_sentence(Dataset):
    def __init__(self, _path, use_sos=False):
        ...
        if not _path: _path = '/home/eric/test/SemanticRL/Europarl'  # change to your own path
        self._path = os.path.join(_path, 'english_vocab.pkl')
        self.dict = {} #'/home/eric/test/SemanticRL/Europarl/english_vocab.pkl'
        tmp = pickle.load(open(self._path, 'rb')) # sentences
        for kk,vv in tmp['voc'].items(): self.dict[kk] = vv+3 # index = number
        # add sos, eos, and pad.
        self.dict['PAD'], self.dict['SOS'], self.dict['EOS'] = 0, 1, 2
        self.len_range = tmp['len_range'] # (3,20)
        self.rev_dict = {vv: kk for kk, vv in self.dict.items()}
        sos_head = [1] if use_sos else []
        self.data_num = torch.tensor([sos_head + list(map(lambda t:self.dict[t], x.split(' '))) + [2] #torch.Size([709588, 21])
    ...
    def __getitem__(self, index):#Pytorch 的數據集模組使用此函數來獲取樣本並構建數據
