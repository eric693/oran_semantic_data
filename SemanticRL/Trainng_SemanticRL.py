...
import os, argparse, yaml, random, time, sys, json
import platform
import re
...
print('\n[*]---------------args init done')
print('args preview:')
for k in args.__dict__:  print(k + ": " + str(args.__dict__[k])) # yaml parameter
...
train_data_loader = data.DataLoader(data_train,**train_loader_params) #return vocabulary size, sentences size
...
def train(encoder, decoder, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (train_sents, len_batch) in enumerate(train_loader):
        ...
        clip_gradient(optimizer, 0.1) #防止梯度爆炸，需要添加在loss反向傳播后，optimizer.step()前
        optimizer.step()#更新參數
